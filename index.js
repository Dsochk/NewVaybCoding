...
const sessionMiddleware = cookieSession({
    name: 'session',
    keys: [process.env.SESSION_KEY || 'my_secret_key'], // Используйте безопасный ключ
    maxAge: 24 * 60 * 60 * 1000, // 24 часа
    secure: true, // Куки отправляются только по HTTPS
    sameSite: 'none', // Для кросс-доменных запросов (если применимо)
...
async function handleRequest(req, res) {
  console.log('Incoming cookies:', req.headers.cookie);
    ...
    // Применяем middleware сессии и ждем его завершения
    ...
    conif (req.url === '/login' && req.method === 'POST') {
        let body = '';
